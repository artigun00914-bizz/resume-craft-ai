...
- BULLET DETAIL & TECHNOLOGY: Write each work experience bullet in more detail, explicitly mentioning the relevant programming languages, frameworks, libraries, cloud platforms, and tools where they truthfully fit the role (e.g., Python, TypeScript, Node.js, React, FastAPI, LangChain, LlamaIndex, PostgreSQL, Redis, Kubernetes, GCP, AWS, Docker, Terraform, Twilio, Dialogflow). Avoid generic phrases like "built systems" or "worked on services" without naming what was actually used. Keep the detail concise and ATS-friendly, and never invent technology outside the role's ground truth.
